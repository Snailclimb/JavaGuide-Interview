...
分布式缓存的话，使用的比较多的主要是 Memcached 和 Redis。现在常用 Redis。
...
![](../../asserts/2021-09-07-22-20-20.png)
...
#### 1.6.1.3. 计数器（字符串的value为整数的时候可以使用）
...
## 1.14. Redis 持久化机制(怎么保证 Redis 挂掉之后再重启数据可以进行恢复)
