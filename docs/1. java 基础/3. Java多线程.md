...
<div align="center">  
<img src="https://my-blog-to-use.oss-cn-beijing.aliyuncs.com/2JVM运行时数据区域.png" width="600px"/>
</div>
...
```
Thread[线程main]get resource1
Thread[线程main]get resource2
Thread[线程main]waiting get resource2
Thread[线程main]waiting get resource1
```
...
## 1.17. 说说 sleep() 方法和 wait() 方法区别和共同点?
...
## 1.18. . 为什么我们调用 start() 方法时会执行 run() 方法，为什么我们不能直接调用 run() 方法？
...
## 1.19. . 说一说自己对于 synchronized 关键字的了解
...
## 1.20. . 说说自己是怎么使用 synchronized 关键字
...
## 1.21. . 构造方法可以使用 synchronized 关键字修饰么？

先说结论：构造方法不能使用 synchronized 关键字修饰。

构造方法本身就属于线程安全的，不存在同步的构造方法一说。

## 1.22. . 讲一下 synchronized 关键字的底层原理

synchronized 关键字底层原理属于 JVM 层面。

## 1.23. .1. synchronized 同步语句块的情况
...
![synchronized关键字原理](https://my-blog-to-use.oss-cn-beijing.aliyuncs.com/2synchronized关键字原理.png)
...
![synchronized关键字原理](https://my-blog-to-use.oss-cn-beijing.aliyuncs.com/2synchronized关键字原理2.png)
...
![JMM(Java内存模型)](https://guide-blog-images.oss-cn-shenzhen.aliyuncs.com/2cb-8d8e-67e8.png)

要解决这个问题，就需要把变量声明为`volatile`，这就指示 JVM，这个变量是共享且不稳定的，每次使用它都到主存中进行读取。

所以，`volatile` 关键字 除了防止 JVM 的指令重排 ，还有一个重要的作用就是保证变量的可见性。

![volatile关键字的可见性](https://guide-blog-images.oss-cn-shenzhen.aliyuncs.com/2d4bf-adad-8aaccf.png)

## 1.28. . 说说 synchronized 关键字和 volatile 关键字的区别
...
![ThreadLocal数据结构](https://guide-blog-images.oss-cn-shenzhen.aliyuncs.com/26.jpeg)

`ThreadLocalMap`是`ThreadLocal`的静态内部类。

![ThreadLocal内部类](https://my-blog-to-use.oss-cn-beijing.aliyuncs.com/2ThreadLocal内部类.png)

## 1.31. . ThreadLocal 内存泄露问题了解不？
...
## 1.34. .2. 实现 Runnable 接口和 Callable 接口的区别
...
## 1.35. .3. 执行 execute()方法和 submit()方法的区别是什么呢？
...
方式一：通过构造方法实现
![ThreadPoolExecutor构造方法](https://my-blog-to-use.oss-cn-beijing.aliyuncs.com/2ThreadPoolExecutor构造方法.png)
...
![Executor框架的工具类](https://my-blog-to-use.oss-cn-beijing.aliyuncs.com/2Executor框架的工具类.png)

## 1.37. .5. ThreadPoolExecutor 类分析
...
### 1.37.1. .5.1. `ThreadPoolExecutor`构造函数重要参数分析
...
### 1.37.2. .5.2. `ThreadPoolExecutor` 饱和策略
...
![JUC原子类概览](https://my-blog-to-use.oss-cn-beijing.aliyuncs.com/2JUC原子类概览.png)

## 1.40. . JUC 包中的原子类是哪 4 类?
...
![AQS类](https://my-blog-to-use.oss-cn-beijing.aliyuncs.com/2AQS类.png)
...
![AQS原理图](https://my-blog-to-use.oss-cn-beijing.aliyuncs.com/2AQS原理图.png)
...
## 1.47. . 用过 CountDownLatch 么？什么场景下用的？
